      ...
      <div className="w-full border-b border-slate-100 px-8 py-8 flex flex-wrap items-center justify-between gap-4 bg-slate-50/30">
        <div className="flex items-center gap-6">
          ...
          <div>
            ...
            <p className="text-[12px] font-black text-slate-400 uppercase tracking-[0.2em] mt-1 italic">{quo.quotationId}</p>
          </div>
        </div>

        <div className="flex items-center gap-3">
          {(quo.approvalStatus === 'Approved') && (
            <span className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-emerald-50 border border-emerald-200 text-[12px] font-black text-emerald-700 uppercase tracking-wider">
              <CheckCircle size={14} /> APPROVED
            </span>
          )}
          {(quo.approvalStatus === 'Rejected') && (
            <span className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-rose-50 border border-rose-200 text-[12px] font-black text-rose-700 uppercase tracking-wider">
              <XCircle size={14} /> REJECTED
            </span>
          )}
          {isPending && (
            <span className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-amber-50 border border-amber-200 text-[12px] font-black text-amber-700 uppercase tracking-wider">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="bg-slate-50 rounded-2xl p-6 border border-slate-100">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <p className="text-[11px] font-black text-slate-400 uppercase tracking-widest">Vendor</p>
                  <p className="text-lg font-black text-slate-800 uppercase mt-1">{quo.vendorName || quo.vendorId}</p>
                  {quo.vendorName && (
                    <p className="text-[11px] font-mono text-slate-400 mt-0.5">{quo.vendorId}</p>
                  )}
                </div>
                <div>
                ...
                <div>
                  <p className="text-[11px] font-black text-slate-400 uppercase tracking-widest">Term of Payment</p>
                ...
                <table className="w-full text-left">
                  ...
                  <tfoot>
                    <tr className="bg-slate-50 border-t border-slate-200">
                      <td colSpan="4" className="px-5 py-3 text-right font-black text-[12px] uppercase text-slate-500">Subtotal Barang</td>
            ...
            {quo.documentUrl && (
              <button
                ...
                className="flex items-center justify-center gap-2 w-full py-3 bg-slate-100 hover:bg-slate-200 rounded-xl text-[12px] font-black uppercase tracking-widest text-slate-600 transition-all"
              >
          ...
          <div className="space-y-6">
            <div className="bg-slate-900 rounded-2xl p-6 text-white shadow-xl sticky top-24">
              ...
              <div className="space-y-3">
                <div className="flex justify-between py-2 border-b border-white/10">
                  <span className="text-[12px] font-bold text-slate-300">Subtotal Barang</span>
                  <span className="font-black text-white">Rp {formatRupiah(subtotalCOGS)}</span>
                </div>
                {additionalFee > 0 && (
                  <div className="flex justify-between py-2 border-b border-white/10">
                    <span className="text-[12px] font-bold text-slate-300">Additional Fee</span>
                ...
                <div className="flex justify-between py-3 mt-2 bg-indigo-500/20 -mx-3 px-3 rounded-xl">
                  <span className="text-[13px] font-black text-indigo-300 uppercase tracking-wider">TOTAL MODAL</span>
              ...
              {quo.approvalStatus === 'Rejected' && quo.rejectionReason && (
                <div className="mt-4 pt-3 border-t border-white/10">
                  <p className="text-[10px] text-rose-400 uppercase font-black tracking-widest">Alasan Ditolak</p>
                  <p className="text-[12px] text-rose-300 mt-1">{quo.rejectionReason}</p>
                </div>
              )}
            </div>
            ...
            {isPending && bolehMenyetujui && (
              <div className="bg-white rounded-2xl border border-slate-200 p-5 space-y-3">
                <p className="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-2">Management Action</p>
                <button
                  onClick={() => handleApprove('Approved')}
                  disabled={processing}
                  className="w-full flex items-center justify-center gap-2 py-3 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl font-black text-[12px] uppercase tracking-widest transition-all disabled:opacity-50"
                >
                ...
                <button
                  ...
                  disabled={processing}
                  className="w-full flex items-center justify-center gap-2 py-3 bg-rose-500 hover:bg-rose-600 text-white rounded-xl font-black text-[12px] uppercase tracking-widest transition-all disabled:opacity-50"
                >
