...
const Dashboard = () => {
  ...
  const { t: tr, lang, tStatus, fmtRupiah, fmtTanggal } = useLang();
  // Kunci kamus Dashboard sudah pindah ke src/i18n/kamus.js. Pembungkus ini
  // menjaga pemanggilan t.namaKunci di JSX di bawah tetap seperti semula.
  const t = new Proxy({}, { get: (_, k) => tr(`dashboard.${String(k)}`) });
  ...
  const fmtNilai = fmtRupiah;

  const allModules = {
    Marketing: [
      { title: 'Project Center', titleId: 'Pusat Project', icon: FolderPlus, desc: 'Tambah project baru & lihat log', descEn: 'Add new projects & view logs', path: '/project-center' },
      { title: 'Create Quotation', titleId: 'Buat Quotation', icon: FileText, desc: 'Buat quotation harga untuk client', descEn: 'Create detailed price quotations', path: '/quotation-center' },
      { title: 'Create Invoice', titleId: 'Buat Invoice', icon: Receipt, desc: 'Buat invoice untuk client', descEn: 'Create invoices for customers', path: '/invoice-center' },
      { title: 'Input Payment', titleId: 'Input Pembayaran', icon: CreditCard, desc: 'Upload bukti transfer client & riwayatnya', descEn: 'Upload client transfer proof & view log', path: '/input-payment-center' },
    ...
    Procurement: [
      { title: 'Vendor Directory', titleId: 'Direktori Vendor', icon: Package, desc: 'Kelola data master supplier', descEn: 'Manage master suppliers', path: '/vendor' },
      ...
      { title: 'Receive & QC Goods', titleId: 'Terima & QC Barang', icon: PackageSearch, desc: 'Terima, cek barang & proses retur', descEn: 'Receive, inspect goods & process returns', path: '/receive-qc', color: 'border-amber-500' },
      { title: 'Invoice Submission', titleId: 'Pengajuan Invoice', icon: FileText, desc: 'Teruskan tagihan vendor ke Finance', descEn: 'Forward vendor invoices to Finance', path: '/supplier-invoice-menu' },
      { title: 'Delivery Management', titleId: 'Manajemen Delivery', icon: Truck, desc: 'Atur jadwal pengiriman ke client', descEn: 'Schedule deliveries to clients', path: '/delivery-management' },
      { title: 'Inventory Storage', titleId: 'Gudang Inventory', icon: Boxes, desc: 'Stok barang dari PO: sisa & terpakai', descEn: 'PO stock: remaining & used', path: '/inventory' },
    ...
    Finance: [
      { title: 'Project Billing', titleId: 'Penagihan Project', icon: Receipt, desc: 'Terbitkan invoice dari quotation', descEn: 'Generate invoices from quotations', path: '/project-billing' },
      { title: 'Client Payment', titleId: 'Pembayaran Client', icon: CreditCard, desc: 'Input pembayaran masuk', descEn: 'Input incoming payments', path: '/finance-payment-center' },
      { title: 'Payment Verification', titleId: 'Verifikasi Pembayaran', icon: Shield, desc: 'Verifikasi bukti bayar client', descEn: 'Verify client payment proofs', path: '/verify-payment' },
      { title: 'Supplier Payment', titleId: 'Pembayaran Supplier', icon: DollarSign, desc: 'Proses pembayaran ke vendor', descEn: 'Process vendor payments', path: '/supplier-payment' },
      { title: 'Project Financial Summary', titleId: 'Ringkasan Keuangan Project', icon: BarChart3, desc: 'Ringkasan pendapatan & biaya per project', descEn: 'Revenue & cost summary per project', path: '/financial-report' },
    ...
    Admin: [
      { title: 'User Management', titleId: 'Manajemen Pengguna', icon: Users, desc: 'Atur akun karyawan', descEn: 'Manage employee accounts', path: '/manage-users' },
      { title: 'System Logs', titleId: 'Log Sistem', icon: Logs, desc: 'Audit aktivitas sistem', descEn: 'Audit system activity', path: '/logs' }
    ],
    Owner: [
      { title: 'Project Financial Summary', titleId: 'Ringkasan Keuangan Project', icon: BarChart3, desc: 'Ringkasan pendapatan & biaya per project', descEn: 'Revenue & cost summary per project', path: '/financial-report' },
      { title: 'Project Timeline', icon: Calendar, desc: 'Pantau progress & milestone project', descEn: 'Monitor progress & milestones', path: '/timeline' }
    ...
    Management: [
      { title: 'Supplier Quotation Approval', titleId: 'Approval Supplier Quotation', icon: FileCheck, desc: 'Approve quotation dari supplier', descEn: 'Approve supplier quotations', path: '/quotation-approval' },
      { title: 'Client Quotation Approval', titleId: 'Approval Client Quotation', icon: FileText, desc: 'Approve quotation untuk client', descEn: 'Approve client quotations', path: '/client-quotation-approval' }
    ]
  };
  ...
  const modulLihatSaja = [
    { modul: 'Project',            title: 'Project Log', titleId: 'Log Project', icon: Logs, desc: 'Daftar seluruh project', descEn: 'All project records', path: '/project-log' },
    ...
    { modul: 'Client Quotation',   title: 'Quotation Log', titleId: 'Log Quotation', icon: FileText, desc: 'Riwayat quotation ke client', descEn: 'Client quotation history', path: '/quotation-log' },
    { modul: 'Client Invoice',     title: 'Invoice Log', titleId: 'Log Invoice', icon: Receipt, desc: 'Riwayat invoice client', descEn: 'Client invoice history', path: '/invoice-log' },
    { modul: 'Client Payment',     title: 'Payment Log', titleId: 'Log Pembayaran', icon: CreditCard, desc: 'Riwayat pembayaran masuk', descEn: 'Incoming payment history', path: '/input-payment-log' },
    { modul: 'Supplier Quotation', title: 'Supplier Quotation Record', titleId: 'Rekaman Supplier Quotation', icon: FileCheck, desc: 'Riwayat quotation supplier', descEn: 'Supplier quotation history', path: '/supplier-quotation-record' },
    { modul: 'Purchase Order',     title: 'PO Record', titleId: 'Rekaman PO', icon: ClipboardCheck, desc: 'Riwayat purchase order', descEn: 'Purchase order history', path: '/po-record' },
    { modul: 'Supplier Invoice',   title: 'Supplier Invoice Record', titleId: 'Rekaman Supplier Invoice', icon: FileText, desc: 'Riwayat tagihan vendor', descEn: 'Vendor invoice history', path: '/supplier-invoice-record' },
    { modul: 'Vendor',             title: 'Vendor Directory', titleId: 'Direktori Vendor', icon: Package, desc: 'Data master supplier', descEn: 'Supplier master data', path: '/existing-vendors' },
    { modul: 'Inventory',          title: 'Inventory', icon: Boxes, desc: 'Stok & mutasi barang', descEn: 'Stock & movements', path: '/inventory' },
    { modul: 'Expense Submission', title: 'Expense Log', titleId: 'Log Expense', icon: DollarSign, desc: 'Riwayat pengajuan biaya', descEn: 'Expense submission history', path: '/expense-submission-log' },
    { modul: 'Financial Report',   title: 'Project Financial Summary', titleId: 'Ringkasan Keuangan Project', icon: BarChart3, desc: 'Ringkasan pendapatan & biaya', descEn: 'Revenue & cost summary', path: '/financial-report' },
    { modul: 'System Logs',        title: 'Activity Log', titleId: 'Log Aktivitas', icon: Logs, desc: 'Jejak aktivitas sistem', descEn: 'System activity trail', path: '/logs' },
  ];
  ...
  // Sapaan Indonesia punya empat tingkat, English hanya tiga; di kamus English
  // greetNoon sengaja disamakan dengan greetAfternoon supaya ambang jamnya
  // tidak perlu bercabang per bahasa di sini.
  const greeting = () => {
    const hour = new Date().getHours();
    if (hour < 11) return tr('dashboard.greetMorning');
    if (hour < 15) return tr('dashboard.greetNoon');
    if (hour < 18) return tr('dashboard.greetAfternoon');
    return tr('dashboard.greetEvening');
  };
          ...
          <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            <div>
              ...
              <p className="text-slate-500 text-xs font-bold mt-2 uppercase tracking-widest">
                {user.role} • {fmtTanggal(new Date(), { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}
              </p>
            </div>

            <div className="flex items-center gap-3">
              {/* Pemilih bahasa pindah ke Header supaya terjangkau dari semua
                  halaman, bukan hanya dari Dashboard. */}
              ...
              {userModules.map((mod, idx) => (
                ...
                >
                  ...
                  <h3 className="text-sm font-black text-slate-800 uppercase tracking-tighter mb-1">
                    {lang === 'en' ? mod.title : (mod.titleId || mod.title)}
                  </h3>
