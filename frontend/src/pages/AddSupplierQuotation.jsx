...
const AddSupplierQuotation = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.projectId || !formData.vendorId) {
       return Swal.fire('Warning', t('sw.projectVendorRequired'), 'warning');
    }

    if (formData.isTaxIncluded && (!formData.taxAmount || Number(formData.taxAmount) <= 0)) {
       return Swal.fire(t('sw.dataIncomplete'), t('sw.taxAmountRequired'), 'warning');
    }
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              ...
              <div className="space-y-1">
                ...
                <input type="text" readOnly disabled value="" placeholder={t('common.autoGenerated')} className="w-full p-3.5 bg-slate-50 border border-slate-200 rounded-xl font-mono text-slate-400 italic font-bold outline-none" />
              </div>
              ...
              {quotationItems.map((item, index) => (
                <div key={index} className="grid grid-cols-1 md:grid-cols-12 gap-4 p-5 bg-slate-50 border border-slate-200 rounded-2xl items-end relative transition-all hover:border-indigo-300">
                  <div className="md:col-span-3 space-y-1">
                    ...
                    <input list={`item-list-${index}`} name="itemName" value={item.itemName} onChange={(e) => handleItemChange(index, e)} required placeholder={t('form.itemName')} className="w-full p-3 bg-white border border-slate-300 rounded-lg text-xs font-bold outline-none focus:border-indigo-600 shadow-sm" />
                  ...
                  <div className="md:col-span-2 space-y-1">
                    <label className="text-xs font-black text-slate-400 uppercase tracking-widest ml-1 italic leading-none mb-1.5">{t('label.pricePerUnit')} ({formData.currency})</label>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="space-y-1">
                ...
                <textarea name="remarks" placeholder={t('form.noteMgmt')} className="w-full p-4 bg-white border border-slate-300 rounded-2xl h-full min-h-[120px] outline-none font-medium text-slate-700 focus:border-indigo-600 shadow-sm transition-all" onChange={handleChange} value={formData.remarks} />
              </div>
