...
const InvoiceLogDetail = () => {
      ...
      default:
        return (
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-amber-50 border border-amber-200">
            ...
            <span className="text-xs font-black text-amber-700 uppercase tracking-wider">UNPAID</span>
          </div>
        );
    }
  };
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      doc.text("TOP", 120, 83);
      doc.text(`: ${invoice.topOption || invoice.billingPhase || 'COD'}`, 150, 83);
      ...
      <div className="w-full border-b border-slate-100 px-8 py-8 flex flex-wrap items-center justify-between gap-4 bg-slate-50/30">
        <div className="flex items-center gap-6">
          ...
          <div>
            ...
            <p className="text-xs font-black text-slate-400 uppercase tracking-[0.2em] mt-1 italic">{invoice.invoiceNumber}</p>
          </div>
        </div>
        <button
          onClick={handleDownloadPDF}
          className="flex items-center gap-2 px-5 py-3 bg-slate-900 text-white rounded-xl text-xs font-black uppercase tracking-widest hover:bg-indigo-700 transition-all"
        >
            ...
            <div className="bg-slate-50 rounded-2xl p-6 border border-slate-100">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-1">ID: {invoice.projectId}</p>
                </div>
                <div>
          ...
          <div className="space-y-6">
            <div className="bg-slate-900 rounded-2xl p-6 text-white shadow-xl sticky top-24">
              ...
              <div className="space-y-3">
                ...
                {taxAmount > 0 && (
                  <div className="flex justify-between py-2 border-b border-white/10">
                    <span className="text-xs font-bold text-slate-300 flex items-center gap-1"><Receipt size={12} /> Tax</span>
