...
import {
  ...
  Boxes, MapPin, Phone, Activity, Banknote
} from 'lucide-react';
...
const ProjectTimeline = () => {
  ...
  const { project, financial, progress, paymentStages, clientInvoices, purchaseOrders, supplierInvoices, cashOut, profitMargin } = data;
        ...
        >
          <div className="px-6 py-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-5">
              ...
              <div className={`${profitMargin.grossProfit >= 0 ? 'bg-emerald-50' : 'bg-red-50'} rounded-xl p-4`}>
                <p className={`text-[8px] font-black uppercase tracking-wider mb-1 ${profitMargin.grossProfit >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                  Gross Profit
                </p>
                <p className={`text-xl font-black ${profitMargin.grossProfit >= 0 ? 'text-emerald-800' : 'text-red-800'}`}>
            ...
            <div className="mb-2">
              <div className="flex items-center justify-between mb-1.5">
                <p className="text-[9px] font-black text-slate-500 uppercase tracking-wider">Margin</p>
                <div className="flex items-center gap-1.5">
                  {profitMargin.grossProfit >= 0
                    ? <TrendingUp size={14} className="text-emerald-500" />
                    : <TrendingDown size={14} className="text-red-500" />}
                  <span className={`text-xl font-black ${profitMargin.grossProfit >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>
              ...
              <div className="w-full h-3 bg-slate-100 rounded-full overflow-hidden">
                <div
                  className={`h-full rounded-full transition-all duration-700 ${profitMargin.grossProfit >= 0 ? 'bg-emerald-500' : 'bg-red-500'}`}
                  style={{ width: `${Math.min(Math.abs(profitMargin.marginPercent), 100)}%` }}
                />
              </div>
              {profitMargin.cogs === 0 && (
