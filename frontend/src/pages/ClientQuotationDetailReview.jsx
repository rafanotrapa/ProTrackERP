  ...
  if (loading) return (
    <div className="min-h-screen bg-white flex items-center justify-center">
      <div className="text-center">
        ...
        <p className="font-black uppercase tracking-widest text-slate-400 text-xs italic">LOADING QUOTATION DATA...</p>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-white flex flex-col font-sans">
      ...
      <div className="p-6 md:p-10 lg:p-12">
        <button
          ...
          className="flex items-center gap-2 text-slate-400 hover:text-slate-900 font-black text-xs uppercase tracking-widest mb-6 transition-all group"
        >
        ...
        <div className="max-w-6xl mx-auto">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8 pb-6 border-b border-slate-100">
            <div>
              <div className="flex items-center gap-3 mb-3 flex-wrap">
                ...
                <span className="text-xs font-black text-indigo-500 uppercase tracking-widest bg-indigo-50 px-3 py-1.5 rounded-full">
                  {quo.quotationId}
                </span>
                <span className={`text-xs font-black px-3 py-1.5 rounded-full uppercase tracking-wider ${
              ...
              <div className="flex flex-wrap gap-4 mt-3 text-xs font-bold text-slate-500 uppercase tracking-widest">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-8">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="bg-slate-50 p-5 rounded-2xl border border-slate-100">
                  <label className="text-xs font-black text-slate-400 uppercase tracking-widest block mb-1">Term of Payment</label>
                  <p className="text-base font-black text-amber-600 uppercase">{quo.topOption || 'N/A'}</p>
                </div>
                <div className="bg-slate-50 p-5 rounded-2xl border border-slate-100">
                  <label className="text-xs font-black text-slate-400 uppercase tracking-widest block mb-1">Currency</label>
                  <p className="text-base font-black text-slate-800 uppercase">{quo.currency || 'IDR'}</p>
                </div>
                <div className="bg-slate-50 p-5 rounded-2xl border border-slate-100">
                  <label className="text-xs font-black text-slate-400 uppercase tracking-widest block mb-1">Items Count</label>
                  <p className="text-base font-black text-slate-800">{quo.items?.length || 0} item(s)</p>
                </div>
                <div className="bg-slate-50 p-5 rounded-2xl border border-slate-100">
                  <label className="text-xs font-black text-slate-400 uppercase tracking-widest block mb-1">Mode</label>
              ...
              <div>
                <h3 className="text-sm font-black text-slate-800 uppercase tracking-[0.3em] flex items-center gap-2 mb-5">
                        ...
                        return (
                          <tr key={index} className="border-b border-slate-100 hover:bg-slate-50/50 transition-colors">
                            ...
                            <td className="py-4 px-3 text-right">
                              <span className={`text-xs font-black px-2 py-1 rounded-full ${itemMargin >= 0 ? 'bg-emerald-100 text-emerald-600' : 'bg-red-100 text-red-600'}`}>
                                {itemMarginPercent}%
                              </span>
                            </td>
                          </tr>
            ...
            <div className="space-y-6">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex justify-between items-center pt-2">
                    ...
                    <div className="text-right">
                      ...
                      <span className={`text-xs font-black ml-2 px-2 py-0.5 rounded-full ${grossProfit >= 0 ? 'bg-emerald-500/30 text-emerald-300' : 'bg-red-500/30 text-red-300'}`}>
                        {marginPercent}%
                      </span>
                    </div>
                  </div>
                ...
                <div className="bg-white border-2 border-slate-100 rounded-3xl p-6 shadow-lg sticky top-6">
                  ...
                  <div className="space-y-3">
                    <button
                      ...
                      className="w-full py-4 bg-emerald-500 hover:bg-emerald-600 rounded-2xl font-black text-sm uppercase tracking-widest transition-all flex items-center justify-center gap-2"
                    >
                      <CheckCircle size={16}/> APPROVE QUOTATION
                    </button>
                    <button
                      onClick={() => handleAction('Rejected')}
                      className="w-full py-4 bg-red-500 hover:bg-red-600 rounded-2xl font-black text-sm uppercase tracking-widest transition-all flex items-center justify-center gap-2"
                    >
                      <XCircle size={16}/> REJECT QUOTATION
                    </button>
                  </div>
                  <p className="text-2xs text-slate-400 text-center mt-4 uppercase tracking-widest">
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    {quo.approvalStatus === 'Rejected' && quo.rejectionReason && (
                      <div className="mt-3 pt-3 border-t border-slate-200">
                        <span className="text-slate-500 block mb-1 text-xs font-black uppercase tracking-widest">Rejection Reason</span>
              ...
              <button
                onClick={() => navigate('/client-quotation-approval')}
                className="w-full py-4 border-2 border-slate-200 rounded-2xl font-black text-xs uppercase tracking-widest text-slate-500 hover:bg-slate-50 transition-all active:scale-95"
              >
