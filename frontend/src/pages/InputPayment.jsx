    ...
    <div className="min-h-screen bg-white flex flex-col font-sans text-slate-900">
      ...
      <div className="w-full border-b border-slate-100 px-8 py-8 flex items-center gap-6 bg-slate-50/30">
        ...
        <div>
          ...
          <p className="text-xs font-black text-slate-400 uppercase tracking-[0.2em] mt-1 italic">Verification Routing System</p>
        </div>
      </div>
        ...
        <form onSubmit={handleSubmit} className="max-w-6xl mx-auto space-y-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
            <div className="space-y-3">
              <label className="text-sm font-black text-slate-400 uppercase tracking-widest italic">1. Select Reference Invoice</label>
              <StyledSelect
            ...
            <div className="space-y-3">
              <label className="text-sm font-black text-slate-400 uppercase tracking-widest italic">2. Actual Payment Date</label>
              <input
                type="date"
                required
          ...
          <div className="p-12 bg-slate-900 rounded-[2.5rem] shadow-2xl relative overflow-hidden text-white border-b-8 border-indigo-600">
            <div className="relative z-10 flex flex-col gap-8">
              <div>
                <label className="text-xs font-black text-indigo-400 uppercase tracking-[0.2em] italic">Confirmed Billing Amount</label>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10 pt-10 border-t border-white/10">
                <div className="space-y-4">
                  <div>
                    <label className="text-xs font-black text-slate-500 uppercase tracking-widest italic">Client Identity</label>
                    <p className="text-lg font-black uppercase leading-none mt-1">{formData.clientName || '-'}</p>
                  </div>
                  <div>
                    <label className="text-xs font-black text-slate-500 uppercase tracking-widest italic">Project Assignment</label>
                    <p className="text-lg font-black uppercase leading-none mt-1">{formData.projectName || '-'}</p>
                  </div>
                </div>
                <div className="md:text-right space-y-4">
                  <div>
                    <label className="text-xs font-black text-slate-500 uppercase tracking-widest italic">Total Contract Value</label>
                    <p className="text-lg font-black leading-none mt-1 text-slate-300">Rp {Number(formData.totalContractValue).toLocaleString()}</p>
                  </div>
                  <div>
                    <label className="text-xs font-black text-slate-500 uppercase tracking-widest italic">Billing Phase / TOP</label>
          ...
          <div className="space-y-3">
            <label className="text-sm font-black text-slate-400 uppercase tracking-widest italic">3. Transaction Evidence (Image / PDF)</label>
          ...
          <div className="flex justify-end pt-10 border-t border-slate-100">
            <button
              type="submit"
              disabled={loading}
              className={`px-12 py-5 rounded-2xl font-black text-white uppercase tracking-[0.2em] text-sm shadow-2xl transition-all active:scale-95 ${
