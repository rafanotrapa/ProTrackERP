...
const AddVendor = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!formData.projectId) return Swal.fire('Warning', t('sw.pickTargetProject'), 'warning');
    ...
    <div className="min-h-screen bg-white font-sans flex flex-col text-slate-900">
      ...
      <div className="w-full border-b border-slate-100 px-8 py-8 flex items-center gap-6 bg-slate-50/30">
        ...
        <div>
          ...
          <p className="text-xs font-black text-slate-400 uppercase tracking-[0.2em] mt-1 italic leading-none">Procurement Module • Partner Onboarding</p>
        </div>
      </div>
        ...
        <form onSubmit={handleSubmit} className="max-w-none w-full space-y-12">
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
              <div className="space-y-1">
                <label className="text-xs font-black text-slate-400 uppercase tracking-widest ml-1 italic leading-none mb-1.5">Vendor ID</label>
                <input type="text" readOnly disabled value="" placeholder={t('common.autoGenerated')} className="w-full p-3.5 bg-slate-50 border border-slate-200 rounded-xl font-mono text-slate-400 italic font-bold outline-none shadow-sm" />
              </div>

              <div className="space-y-1 md:col-span-2">
                <label className="text-xs font-black text-slate-400 uppercase tracking-widest ml-1 italic leading-none mb-1.5">Project Target</label>
                <StyledSelect
                  name="projectId"
              ...
              <div className="space-y-1">
                <label className="text-xs font-black text-slate-400 uppercase tracking-widest ml-1 italic leading-none mb-1.5">Type</label>
                <StyledSelect
                  name="companyType"
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
               <div className="space-y-1 md:col-span-2">
                 <label className="text-xs font-black text-slate-400 uppercase tracking-widest ml-1 italic leading-none mb-1.5">Legal Company Name</label>
               ...
               <div className="space-y-1">
                 <label className="text-xs font-black text-slate-400 uppercase tracking-widest ml-1 italic leading-none mb-1.5">Category</label>
                 <input
                   name="category"
                   type="text"
                   placeholder={t('ph.vendorCategory')}
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="space-y-1">
                <label className="text-xs font-black text-slate-400 uppercase tracking-widest ml-1 italic leading-none mb-1.5">Email Address</label>
              ...
              <div className="space-y-1">
                <label className="text-xs font-black text-slate-400 uppercase tracking-widest ml-1 italic leading-none mb-1.5">Phone / WhatsApp</label>
                ...
                <p className="text-2xs text-slate-400 mt-1 ml-1">{t('form.digitsOnly')}</p>
              </div>
            </div>
            <div className="space-y-1 w-full">
              <label className="text-xs font-black text-slate-400 uppercase tracking-widest ml-1 italic leading-none mb-1.5">Full Office Address</label>
              <textarea name="address" placeholder={t('ph.officeAddress')} className="w-full p-5 bg-white border border-slate-300 rounded-2xl h-28 outline-none font-medium text-slate-700 focus:border-indigo-600 shadow-sm transition-all" onChange={handleChange} />
            </div>
          </div>
          ...
          <div className="flex justify-end pt-10 border-t border-slate-100">
            <button
              type="submit"
              disabled={loading}
              className={`px-12 py-4 rounded-xl font-black text-white uppercase tracking-widest text-xs shadow-lg transition-all active:scale-95 ${
