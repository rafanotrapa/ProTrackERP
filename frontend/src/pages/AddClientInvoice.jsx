    ...
    <div className="min-h-screen bg-white font-sans flex flex-col">
      ...
      <div className="w-full border-b border-slate-100 px-8 py-8 flex items-center gap-6">
        ...
        <div>
          ...
          <p className="text-xs font-black text-slate-400 uppercase tracking-[0.2em] mt-1 italic">Finance Module • Billing & Collection</p>
        </div>
      </div>
        ...
        <form onSubmit={handleSubmit} className="max-w-6xl space-y-10">
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-1">
                ...
                />
                <p className="text-2xs text-slate-400 italic mt-1">
                  * Only projects with approved client quotation and unpaid balance are shown
                </p>
              </div>
              <div className="space-y-1">
                <label className="text-xs font-black text-slate-400 uppercase tracking-widest ml-1 italic">Invoice Number</label>
          ...
          <div className="space-y-6">
            ...
            <div className="p-8 rounded-[2.5rem] bg-slate-50 border-2 border-slate-100 space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                <div>
                  <label className="text-xs font-black text-indigo-400 uppercase tracking-widest">Client Name</label>
                  <div className="text-2xl font-black text-slate-800 italic uppercase leading-tight">{formData.clientName || '---'}</div>
                </div>
                <div>
                  <label className="text-xs font-black text-indigo-400 uppercase tracking-widest">Project Name</label>
                  <div className="text-xl font-bold text-slate-600 italic leading-tight">{formData.projectName || '---'}</div>
                </div>
                <div>
                  <label className="text-xs font-black text-indigo-400 uppercase tracking-widest">Term of Payment</label>
                  <div className="text-xl font-black text-amber-500 italic uppercase leading-tight">{formData.topOption || '---'}</div>
                </div>
              </div>

              <div className="border-t border-slate-200 pt-6">
                <label className="text-xs font-black text-indigo-400 uppercase tracking-widest mb-4 block">Itemized Billing (with Sales Price)</label>
                  ...
                  {formData.items.length > 0 ? formData.items.map((item, idx) => (
                    <div key={idx} className="flex justify-between items-center bg-white p-5 rounded-2xl border border-slate-100 shadow-sm">
                      ...
                      <div className="text-right">
                        ...
                        {item.cogs && (
                          <p className="text-2xs text-slate-400 line-through mt-1">COGS: Rp {item.cogs.toLocaleString()}</p>
                        )}
                      </div>
                    </div>
                  )) : <p className="text-xs italic text-slate-400 uppercase font-black tracking-widest">Awaiting data...</p>}
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="space-y-1">
                <label className="text-xs font-black text-slate-400 uppercase tracking-widest ml-1 italic">Payment Due Date</label>
                <input
                  type="date"
                  required
              ...
              <div className="md:col-span-2 space-y-1">
                <label className={`text-xs font-black uppercase tracking-widest ml-1 italic underline decoration-emerald-500/30 ${
                ...
                <div className="relative">
                  ...
                  <div className="flex justify-between items-center mt-2 px-1">
                    ...
                    <p className="text-xs font-black text-indigo-500 uppercase italic tracking-tighter bg-indigo-50 px-2 py-0.5 rounded-md">
