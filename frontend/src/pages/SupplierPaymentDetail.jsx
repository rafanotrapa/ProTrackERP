    ...
    if (status === 'Paid') {
      return (
        <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-emerald-50 border border-emerald-200">
          ...
          <span className="text-xs font-black text-emerald-700 uppercase tracking-wider">PAID</span>
        </div>
      );
    }
    return (
      <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-amber-50 border border-amber-200">
        ...
        <span className="text-xs font-black text-amber-700 uppercase tracking-wider">PENDING VERIFICATION</span>
      </div>
    );
  };
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="font-black text-slate-400 text-xs uppercase tracking-widest">LOADING INVOICE...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="w-full border-b border-slate-100 px-8 py-8 flex flex-wrap items-center justify-between gap-4 bg-slate-50/30">
        ...
        {isPending && (
          <button
            ...
            className="flex items-center gap-2 px-5 py-3 bg-emerald-600 text-white rounded-xl text-xs font-black uppercase tracking-widest hover:bg-emerald-700 transition-all shadow-lg shadow-emerald-100"
          >
            <CreditCard size={14} /> Confirm Payment
          </button>
        )}
        {isPaid && (
          <div className="flex items-center gap-2 px-5 py-3 bg-emerald-50 text-emerald-700 rounded-xl text-xs font-black uppercase tracking-widest">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="bg-slate-50 rounded-2xl p-6 border border-slate-100">
              <h3 className="text-sm font-black text-slate-400 uppercase tracking-[0.3em] mb-4 flex items-center gap-2">
                <Building2 size={14} /> Vendor Information
              </h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <p className="text-xs font-black text-slate-400 uppercase tracking-widest">Vendor Name</p>
                ...
                <div>
                  <p className="text-xs font-black text-slate-400 uppercase tracking-widest">PO Number</p>
            ...
            <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
              ...
              <div className="p-5 space-y-3">
                <div className="flex justify-between">
                  <span className="text-xs font-black text-slate-400">Submission ID</span>
                  <span className="text-xs font-bold text-slate-600">{invoice.submissionId || '-'}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-xs font-black text-slate-400">Submitted By</span>
                  <span className="text-xs font-bold text-slate-600">
                    {invoice.user?.name || invoice.user?.username || 'Procurement'}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-xs font-black text-slate-400">Submitted Date</span>
                  <span className="text-xs font-bold text-slate-600">
                ...
                {invoice.dueDate && (
                  <div className="flex justify-between">
                    <span className="text-xs font-black text-slate-400">Due Date</span>
                ...
                {invoice.bankInfo && (
                  <div className="flex justify-between">
                    <span className="text-xs font-black text-slate-400">Bank Info</span>
                    <span className="text-xs font-bold text-slate-600">{invoice.bankInfo}</span>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="bg-slate-900 rounded-2xl p-6 text-white shadow-xl sticky top-24">
              ...
              {isPaid && invoice.paymentDate && (
                <div className="mt-4 pt-3 border-t border-white/10 text-center">
                  <p className="text-2xs text-slate-400">Payment Date</p>
              ...
              <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
                ...
                <div className="p-5">
                  {showFile ? (
                    ...
                    <div className="flex items-center justify-between p-4 bg-slate-50 rounded-xl">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-xs font-black text-slate-700 break-all">{invoice.file}</p>
                  ...
                  <button
                    ...
                    className="mt-3 flex items-center justify-center gap-2 w-full py-2 bg-slate-100 hover:bg-slate-200 rounded-xl text-xs font-black text-slate-600 transition-all"
                  >
              ...
              <div className="bg-white rounded-2xl border border-emerald-200 overflow-hidden">
                ...
                <div className="p-5">
                  ...
                  <button
                    ...
                    className="mt-3 flex items-center justify-center gap-2 w-full py-2 bg-emerald-100 hover:bg-emerald-200 rounded-xl text-xs font-black text-emerald-700 transition-all"
                  >
                    <Download size={12} /> Download / Open in New Tab
                  </button>
                </div>
