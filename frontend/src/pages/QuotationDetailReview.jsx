      ...
      <div className="w-full border-b border-slate-100 px-8 py-8 flex flex-wrap items-center justify-between gap-4 bg-slate-50/30">
        <div className="flex items-center gap-6">
          ...
          <div>
            ...
            <p className="text-xs font-black text-slate-400 uppercase tracking-[0.2em] mt-1 italic">{quo.quotationId}</p>
          </div>
        </div>

        <div className="flex items-center gap-3">
          {(quo.approvalStatus === 'Approved') && (
            <span className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-emerald-50 border border-emerald-200 text-xs font-black text-emerald-700 uppercase tracking-wider">
              <CheckCircle size={14} /> APPROVED
            </span>
          )}
          {(quo.approvalStatus === 'Rejected') && (
            <span className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-rose-50 border border-rose-200 text-xs font-black text-rose-700 uppercase tracking-wider">
              <XCircle size={14} /> REJECTED
            </span>
          )}
          {isPending && (
            <span className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-amber-50 border border-amber-200 text-xs font-black text-amber-700 uppercase tracking-wider">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="bg-slate-50 rounded-2xl p-6 border border-slate-100">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <p className="text-xs font-black text-slate-400 uppercase tracking-widest">Vendor</p>
                  <p className="text-lg font-black text-slate-800 uppercase mt-1">{quo.vendorName || quo.vendorId}</p>
                  {quo.vendorName && (
                    <p className="text-xs font-mono text-slate-400 mt-0.5">{quo.vendorId}</p>
                  )}
                </div>
                <div>
          ...
          <div className="space-y-6">
            ...
            {isPending && bolehMenyetujui && (
              <div className="bg-white rounded-2xl border border-slate-200 p-5 space-y-3">
                <p className="text-xs font-black text-slate-400 uppercase tracking-widest mb-2">Management Action</p>
                <button
                  onClick={() => handleApprove('Approved')}
                  disabled={processing}
                  className="w-full flex items-center justify-center gap-2 py-3 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl font-black text-xs uppercase tracking-widest transition-all disabled:opacity-50"
                >
                ...
                <button
                  ...
                  disabled={processing}
                  className="w-full flex items-center justify-center gap-2 py-3 bg-rose-500 hover:bg-rose-600 text-white rounded-xl font-black text-xs uppercase tracking-widest transition-all disabled:opacity-50"
                >
