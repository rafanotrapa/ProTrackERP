    ...
    <div className="min-h-screen bg-white font-sans text-slate-900 flex flex-col">
      ...
      <main className="flex-1 p-8 md:p-16 flex flex-col lg:flex-row gap-16 max-w-7xl mx-auto w-full">
        <div className="lg:w-2/5 space-y-8">
            <button onClick={() => navigate('/client-invoice')} className="flex items-center gap-2 text-xs font-black uppercase text-slate-400 hover:text-indigo-600 transition-colors">
            ...
            <div className="bg-slate-900 rounded-[3rem] p-10 text-white relative overflow-hidden shadow-2xl">
                ...
                <div className="relative z-10 space-y-8">
                    <div className="flex justify-between items-start">
                        <div><p className="text-xs font-black uppercase text-slate-400 italic mb-1 tracking-widest">Target Invoice</p><p className="text-2xl font-black italic tracking-tighter">{invoice?.invoiceNumber}</p></div>
                        <CreditCard className="text-indigo-400" size={32} />
                    </div>
                    <div><p className="text-xs font-black uppercase text-slate-400 italic mb-1 tracking-widest">Client Name</p><p className="text-xl font-bold uppercase truncate">{invoice?.clientName}</p></div>
                    <div className="pt-8 border-t border-slate-800"><p className="text-xs font-black uppercase text-indigo-400 italic mb-1 tracking-widest">Total Amount</p><p className="text-4xl font-black italic tracking-tighter">Rp {invoice?.amount?.toLocaleString('id-ID')}</p></div>
                </div>
            </div>
        </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="space-y-2">
                        <label className="text-xs font-black uppercase italic tracking-widest text-slate-500">Destination Bank</label>
                    ...
                    <div className="space-y-2">
                        <label className="text-xs font-black uppercase italic tracking-widest text-slate-500">Reference Number</label>
                        <div className="relative">
                ...
                <div className="space-y-2">
                    <label className="text-xs font-black uppercase italic tracking-widest text-slate-500">Amount Received (IDR)</label>
                    <div className="relative">
                ...
                <div className="space-y-2">
                    <label className="text-xs font-black uppercase italic tracking-widest text-slate-500">Evidence / Transfer Slip</label>
                    <div className="relative group h-48 border-4 border-dashed border-slate-200 rounded-4xl bg-white flex flex-col items-center justify-center transition-all hover:border-indigo-600 overflow-hidden">
                        ...
                        <div className="flex flex-col items-center gap-3">
                            ...
                            <p className="text-sm font-black uppercase italic text-slate-900 truncate max-w-50">{formData.file ? formData.file.name : 'Drop Slip Proof'}</p>
                        </div>
                    </div>
                </div>
